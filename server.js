...
app.post('/cv-score', async (req, res) => {
  ...
  const { cvBase64, cvMediaType, jdText, jobTitle, companyName, additionalContext } = req.body;
...
2. Professional Summary — full rewrite using JD vocabulary
   CRITICAL: Write as exactly TWO paragraphs separated by a blank line.
   Paragraph 1: Who you are, years of experience, core strength, key metrics (2-3 sentences)
   Paragraph 2: What makes you distinctive for THIS role specifically — tools, methods, approach (2-3 sentences)
  ...
  try {
    ...
    const doc = new Document({
      ...
      sections: [{ properties: { page: { size: { width: 11906, height: 16838 }, margin: { top: 1000, right: 1000, bottom: 1000, left: 1000 } } }, children: [
        ...
        new Paragraph({ spacing: { before: 80, after: 80 }, children: [new TextRun({ text: '6Sense · Demandbase · HubSpot · Zapier · Make.com · Airtable · Notion · ZoomInfo · Clearbit · Clay · Apollo · Google Analytics · LinkedIn Ads · Programmatic Advertising · Sales Navigator · Lusha · Waalaxy', size: 20, color: '333333', font: 'Arial' })] }),
...
// ── CV EDIT ENDPOINT ──────────────────────────────────────────
// Edits the candidate's original Word CV template
// Only rewrites Professional Summary, Core Expertise, Experience bullets
// Everything else (header, photo, metrics table) preserved exactly

app.post('/cv-edit', async (req, res) => {
  const ip = req.headers['x-forwarded-for']?.split(',')[0] || req.ip || 'unknown';
  if (!checkRateLimit(ip)) return res.status(429).json({ error: 'Too many requests.' });
  ...
  const { cvBase64, cvMediaType, templateBase64, jdText, jobTitle, companyName, additionalContext } = req.body;
  if (!cvBase64) return res.status(400).json({ error: 'CV is required.' });
  ...
  const addCtx = additionalContext
    ? `\nADDITIONAL CONTEXT FROM CANDIDATE — treat as highest priority. Weave naturally into rewrites:\n${additionalContext}`
    : '';
...
Rewrite the CV sections now. Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    const tmpIn = path.join(tmpDir, `cv_in_${Date.now()}.docx`);
    const tmpOut = path.join(tmpDir, `cv_out_${Date.now()}.docx`);
    ...
    // Build replacement text blocks
    // We replace text content of specific paragraphs by finding their markers
    // Strategy: find paragraph containing "PROFESSIONAL SUMMARY" heading text
    // then replace the following paragraph(s) text content
    ...
    // Build new summary XML — two paragraphs
    const summaryParts = (cvData.summary || '').split('\n\n').filter(p => p.trim());
    const summaryParas = summaryParts.map(para =>
      `<w:p><w:pPr><w:pStyle w:val="Normal"/><w:spacing w:after="160"/></w:pPr><w:r><w:t xml:space="preserve">${escXml(para.trim())}</w:t></w:r></w:p>`
    ).join('\n');

    // Build new expertise XML — single paragraph
    const expertisePara = `<w:p><w:pPr><w:pStyle w:val="Normal"/><w:spacing w:after="160"/></w:pPr><w:r><w:t xml:space="preserve">${escXml(cvData.expertise || '')}</w:t></w:r></w:p>`;

    // Use regex to replace content between section headings
    // This finds content after "PROFESSIONAL SUMMARY" until "CORE EXPERTISE"
    docXml = docXml.replace(
      /(<w:t[^>]*>PROFESSIONAL SUMMARY<\/w:t>[\s\S]*?<\/w:p>)([\s\S]*?)(<w:t[^>]*>CORE EXPERTISE<\/w:t>)/i,
      `$1\n${summaryParas}\n$3`
    );

    // Replace content between "CORE EXPERTISE" and "PROFESSIONAL EXPERIENCE"  
    docXml = docXml.replace(
      /(<w:t[^>]*>CORE EXPERTISE<\/w:t>[\s\S]*?<\/w:p>)([\s\S]*?)(<w:t[^>]*>PROFESSIONAL EXPERIENCE<\/w:t>)/i,
      `$1\n${expertisePara}\n$3`
    );
    ...
    return res.status(500).json({ error: 'Unexpected error editing CV. Please try again.' });
  }
});
