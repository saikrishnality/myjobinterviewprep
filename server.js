...
Rewrite the CV sections now. Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    // Step 2 — Edit Word template with python-docx (preserves formatting + tracked changes)
...
// ── PYTHON-DOCX RUNNER ────────────────────────────────────────
// Calls cv_editor.py to edit the Word template using python-docx
let pythonDocxInstalled = false;

async function ensurePythonDocx() {
  if (pythonDocxInstalled) return;
  ...
  try {
    await execFileAsync('python3', ['-c', 'import docx']);
    pythonDocxInstalled = true;
    return;
  } catch(e) {}
  
  // Try installing with different pip commands
  const pipCmds = [
    ['pip3', ['install', 'python-docx', '--break-system-packages', '-q']],
    ['pip', ['install', 'python-docx', '--break-system-packages', '-q']],
    ['python3', ['-m', 'pip', 'install', 'python-docx', '--break-system-packages', '-q']],
    ['python3', ['-m', 'pip', 'install', 'python-docx', '-q', '--user']],
  ];
  ...
  for (const [cmd, args] of pipCmds) {
    try {
      ...
      await execFileAsync('python3', ['-c', 'import docx']);
      pythonDocxInstalled = true;
      console.log('python-docx installed successfully via', cmd);
      return;
    } catch(e) {
      console.log(`${cmd} failed:`, e.message.substring(0, 100));
    }
  }
  throw new Error('Could not install python-docx. Please install it on the server.');
}
...
async function runPythonEditor(templateBase64, cvData, companyName, jobTitle, tracked=false) {
  ...
  const ts = Date.now();
  const tmpTemplate = path.join(tmpDir, `cv_tmpl_${ts}.docx`);
  const tmpOutput = path.join(tmpDir, `cv_out_${ts}.docx`);
  const tmpData = path.join(tmpDir, `cv_data_${ts}.json`);

  fs.writeFileSync(tmpTemplate, Buffer.from(templateBase64, 'base64'));
  fs.writeFileSync(tmpData, JSON.stringify(cvData));

  const scriptPath = path.join(__dirname, 'cv_editor.py');
  const args = [scriptPath, tmpTemplate, tmpOutput, tmpData];
  if (tracked) args.push('tracked');

  try {
    await execFileAsync('python3', args, { timeout: 60000 });
    const outputBuffer = fs.readFileSync(tmpOutput);
    return outputBuffer;
  } finally {
    try { fs.unlinkSync(tmpTemplate); } catch(e) {}
    try { fs.unlinkSync(tmpData); } catch(e) {}
