...
2. Professional Summary — make SURGICAL EDITS to the original summary, do not rewrite from scratch.
   ...
   Separate the two paragraphs with the literal text: [PARAGRAPH BREAK]
   Example: "First paragraph text here. [PARAGRAPH BREAK] Second paragraph text here."
...
%%CV_START%%
{
  ...
  "summary": "paragraph one text. [BLANK LINE] paragraph two text.",
...
Rewrite the CV sections now. Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    const apiData = await apiResponse.json();
    if (!apiResponse.ok) {
      console.error('Claude API error:', JSON.stringify(apiData).substring(0, 300));
      return res.status(apiResponse.status).json({ error: apiData.error?.message || 'Claude API error: ' + apiResponse.status });
    }

    const rawText = apiData.content?.[0]?.text || '';
    console.log('Claude response length:', rawText.length, 'starts with:', rawText.substring(0, 80));
    const jsonMatch = rawText.match(/%%CV_START%%\s*([\s\S]*?)\s*%%CV_END%%/);
    if (!jsonMatch) {
      console.error('No JSON delimiters found in response:', rawText.substring(0, 300));
      return res.status(500).json({ error: 'CV format error — Claude did not return expected format. Please try again.' });
    }
    ...
    try { cvData = JSON.parse(jsonMatch[1]); }
      ...
      let cleaned = jsonMatch[1]
        .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, ' ')
        .replace(/,([\s]*[}\]])/g, '$1')
        .replace(/([^\\])\n/g, '$1 ')   // replace unescaped newlines inside strings
        .replace(/([^\\])\t/g, '$1 ');  // replace unescaped tabs
      try { cvData = JSON.parse(cleaned); }
      catch (e2) {
        // Last resort — extract individual fields
        console.error('JSON parse failed, raw:', jsonMatch[1].substring(0, 200));
        return res.status(500).json({ error: 'Could not parse CV update. The response may have been too long.' });
      }
    }
    ...
    const sumParts = (cvData.summary||'').split(/\[PARAGRAPH BREAK\]|\n\n+/).filter(p=>p.trim());
    ...
    // Build output — tracked changes if template + tracked flag, else clean doc
    const wantsTracked = req.body.tracked === true || req.body.tracked === 'true';
    const hasTemplate  = !!(req.body.templateBase64);
    const suffix = wantsTracked ? '_tracked' : '';

    let outputBuffer;
    if (hasTemplate) {
      outputBuffer = await runPythonEditor(req.body.templateBase64, cvData, companyName, jobTitle, wantsTracked);
    } else {
      outputBuffer = await Packer.toBuffer(doc);
    }

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
    res.setHeader('Content-Disposition', `attachment; filename="CV_${safeCo}_${safeJob}${suffix}.docx"`);
    res.send(outputBuffer);

  } catch (err) {
    console.error('CV update error:', err);
    return res.status(500).json({ error: 'CV update failed: ' + (err.message || String(err)) });
  }
});
...
Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    const sumParts2 = (cvData.summary||'').split(/\[PARAGRAPH BREAK\]|\n\n+/).filter(p=>p.trim());
...
async function runPythonEditor(templateBase64, cvData, companyName, jobTitle, tracked = false) {
  ...
  if (sumStart > -1 && expIdx > -1) {
    const parts = (cvData.summary || '').split(/\[PARAGRAPH BREAK\]|\n\n+/).filter(Boolean);
    let pi = 0;
