...
Rewrite the CV sections now. Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    // Return cvData so extension can reuse it for tracked changes (avoids second Claude call)
    res.setHeader('X-CV-Data', Buffer.from(JSON.stringify(cvData)).toString('base64'));
    res.setHeader('Access-Control-Expose-Headers', 'X-Human-Score, X-CV-Data');
...
app.post('/cv-tracked', async (req, res) => {
  ...
  const { templateBase64, jobTitle, companyName, cvDataJson } = req.body;
  if (!templateBase64) return res.status(400).json({ error: 'Word template is required.' });
  if (!cvDataJson)     return res.status(400).json({ error: 'CV data is required. Please generate a clean CV first.' });

  try {
    // Use the cvData already generated by cv-update — no second Claude call needed
    let cvData;
    try { cvData = JSON.parse(Buffer.from(cvDataJson, 'base64').toString('utf8')); }
    catch(e) { return res.status(400).json({ error: 'Invalid CV data. Please generate a clean CV first.' }); }
