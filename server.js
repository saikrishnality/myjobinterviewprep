...
RULES:
- Use JD language throughout — mirror their exact vocabulary
- Never fabricate experience or skills not in the original CV
- Write out the full rewritten text — not descriptions of what to change
- PRESERVE ALL sections and ALL experience roles — do not drop any
- Only rewrite sections that need changing — keep everything else exactly as is
- The candidate's complete work history must appear in full

WHAT TO REWRITE:
1. Professional headline — update to mirror JD language
2. Professional Summary — full rewrite using JD vocabulary
   CRITICAL: Write as exactly TWO complete paragraphs separated by a blank line.

   Paragraph 1: Who the candidate is, years of experience, domain strengths, key metrics.
   Write as a confident factual statement. No reference to the target company in this paragraph.

   Paragraph 2: What specific experience the candidate brings that is relevant to this role.
   This paragraph describes PAST WORK at PREVIOUS EMPLOYERS — never at the target company.

   ABSOLUTE RULES FOR PARAGRAPH 2 — violations make the CV unusable:

   NEVER name the target company's products, teams, or internal structure.
   WRONG: "directly applicable to owning NxtWave's Academy and NIAT products"
   RIGHT: "directly applicable to edtech demand generation and funnel ownership"

   NEVER use "directly applicable to [company name]" — use the role or domain instead.
   WRONG: "directly applicable to PingCAP's developer community events"
   RIGHT: "directly applicable to developer community event marketing"

   NEVER use "Has led" or "Has built" — use simple past tense.
   WRONG: "Has led multi-channel demand generation"
   RIGHT: "Led multi-channel demand generation"

   NEVER say "for [company]" when the work was done at a previous employer.

   The paragraph must read as: Here is what I have done before. Here is why it is relevant to this type of role.
   NOT as: Here is what I will do for your specific products and teams.

   Do not merge into one block. Two clear paragraphs.
   Complete both paragraphs fully. Do not truncate or trail off mid-sentence.
3. Core Expertise — replace with JD-aligned terms
4. Experience roles — rewrite bullets for the most relevant 2-3 roles only
   ALL other experience roles must be included unchanged

INCLUDE ALL THESE SECTIONS IN OUTPUT:
- headline
- summary  
- expertise
- experience array — EVERY role from the CV, not just the top ones
  For roles you rewrote: include updated bullets
  For roles you did not rewrite: copy the original bullets exactly
...
%%CV_START%%
{
  ...
  "expertise": "comma separated expertise items",
  "experience": [
    {
...
3. Rewrite the CV sections to align with what this company would value most
...
- Two paragraph summary — see rules below
- Preserve all experience roles
- Use JD language — but infer the language from company type
- Output valid JSON between %%CV_START%% and %%CV_END%%

SUMMARY RULES — CRITICAL:
Paragraph 1: Who the candidate is, experience, key metrics. No target company mentioned.
Paragraph 2: Specific past experience relevant to this type of role.
...
- Describe what was done and why it is relevant to THIS TYPE of role
...
app.post('/cv-tracked', async (req, res) => {
  ...
  const { cvBase64, cvMediaType, templateBase64, jdText, jobTitle, companyName, additionalContext, premiumInsights } = req.body;
  if (!cvBase64) return res.status(400).json({ error: 'CV is required.' });
  ...
  // Step 1 — Get rewritten content from Claude
  const addCtx = additionalContext
    ? `\nADDITIONAL CONTEXT: ${additionalContext}`
    : '';
  const premCtx = premiumInsights
    ? `\nLINKEDIN PREMIUM DATA: ${premiumInsights.topSkills || ''} ${premiumInsights.howYouCompare || ''}`
    : '';
...
CONTEXT FOR SUMMARY GENERATION:
Role type: ${roleType}
Industry context (first 600 chars of JD):
${jdText.substring(0, 600)}

CONTEXT FOR EXPERIENCE AND EXPERTISE:
Job Title: ${jobTitle || 'Not provided'}
Company: ${companyName || 'Not provided'}
Full Job Description:
${jdText}${addCtx}${premCtx}

IMPORTANT: Summary must be written for the ROLE TYPE only. Do not mention the target company name in the summary.
...
Rewrite the CV sections now. Return only the JSON between %%CV_START%% and %%CV_END%%.`;
  ...
  try {
    ...
    // Step 2 — Edit Word template with tracked changes
    const fs = require('fs');
    const os = require('os');
    const path = require('path');
    ...
    const tmpIn = path.join(tmpDir, `cv_tracked_in_${Date.now()}.docx`);
    const tmpOut = path.join(tmpDir, `cv_tracked_out_${Date.now()}.docx`);

    fs.writeFileSync(tmpIn, Buffer.from(templateBase64, 'base64'));

    const zip = new AdmZip(tmpIn);
    ...
    // Build a tracked change paragraph — replaces entire paragraph content
    // oldText = original text, newText = replacement text
    // Returns XML for a paragraph with del+ins tracked changes
    const trackedParagraph = (oldText, newText, pPrXml = '') => {
      const delId = nextId();
      const insId = nextId();
      return `<w:p>${pPrXml}<w:del w:id="${delId}" w:author="CV Match" w:date="${DATE}"><w:r><w:delText xml:space="preserve">${esc(oldText)}</w:delText></w:r></w:del><w:ins w:id="${insId}" w:author="CV Match" w:date="${DATE}"><w:r><w:t xml:space="preserve">${esc(newText)}</w:t></w:r></w:ins></w:p>`;
    };

    // Build tracked bullet — for experience bullet points
    const trackedBullet = (oldText, newText, numPrXml = '') => {
      const pPr = numPrXml
        ? `<w:pPr>${numPrXml}<w:rPr><w:del w:id="${nextId()}" w:author="CV Match" w:date="${DATE}"/></w:rPr></w:pPr>`
        : '';
      const delId = nextId();
      const insId = nextId();
      return `<w:p><w:pPr>${numPrXml}</w:pPr><w:del w:id="${delId}" w:author="CV Match" w:date="${DATE}"><w:r><w:delText xml:space="preserve">${esc(oldText)}</w:delText></w:r></w:del><w:ins w:id="${insId}" w:author="CV Match" w:date="${DATE}"><w:r><w:t xml:space="preserve">${esc(newText)}</w:t></w:r></w:ins></w:p>`;
    };

    // ── Find and replace sections in the XML ────────────────
    // Strategy: find section heading, then replace the N paragraphs after it

    // Helper — extract text from a paragraph XML block
    const getParaText = (paraXml) => {
      const matches = paraXml.match(/<w:t[^>]*>([^<]*)<\/w:t>/g) || [];
      return matches.map(m => m.replace(/<[^>]+>/g, '')).join('');
    };

    // Split XML into paragraphs
    const splitParas = (xml) => {
      const parts = [];
      let depth = 0;
      let start = 0;
      let i = 0;
      while (i < xml.length) {
        if (xml.startsWith('<w:p ', i) || xml.startsWith('<w:p>', i)) {
          start = i;
          depth = 1;
          i += 4;
          while (i < xml.length && depth > 0) {
            if (xml.startsWith('<w:p', i) && !xml.startsWith('<w:pPr', i) && !xml.startsWith('<w:pStyle', i)) depth++;
            if (xml.startsWith('</w:p>', i)) { depth--; if (depth === 0) { parts.push({ start, end: i + 6, xml: xml.substring(start, i + 6) }); i += 6; break; } }
            i++;
          }
        } else { i++; }
      }
    ...
    const paras = splitParas(docXml);

    // Find summary paragraphs — after "PROFESSIONAL SUMMARY" heading
    let summaryStartIdx = -1;
    let expertiseStartIdx = -1;
    let expertiseEndIdx = -1;

    paras.forEach((p, idx) => {
      const text = getParaText(p.xml).toUpperCase();
      if (text.includes('PROFESSIONAL SUMMARY') && summaryStartIdx === -1) summaryStartIdx = idx;
      if ((text.includes('CORE EXPERTISE') || text.includes('EXPERTISE')) && summaryStartIdx > -1 && expertiseStartIdx === -1) expertiseStartIdx = idx;
      if (text.includes('PROFESSIONAL EXPERIENCE') && expertiseStartIdx > -1 && expertiseEndIdx === -1) expertiseEndIdx = idx;
    });

    // Build replacement map: paraIndex -> new XML
    const replacements = {};

    // Replace summary paragraphs (between summary heading and expertise heading)
    if (summaryStartIdx > -1 && expertiseStartIdx > -1) {
      const summaryParas = paras.slice(summaryStartIdx + 1, expertiseStartIdx);
      const summaryParts = (cvData.summary || '').split(/\n\n+/).filter(p => p.trim());
      summaryParas.forEach((p, i) => {
        const oldText = getParaText(p.xml);
        if (oldText.trim().length < 3) return; // skip empty spacer paragraphs
        const newText = summaryParts[i] || summaryParts[summaryParts.length - 1] || cvData.summary || '';
        if (oldText.trim() && newText.trim() && oldText.trim() !== newText.trim()) {
          replacements[summaryStartIdx + 1 + i] = trackedParagraph(oldText, newText);
        }
      });
    }

    // Replace expertise paragraph (single paragraph after expertise heading)
    if (expertiseStartIdx > -1 && expertiseEndIdx > -1) {
      const expertisePara = paras[expertiseStartIdx + 1];
      if (expertisePara) {
        const oldText = getParaText(expertisePara.xml);
        const newText = cvData.expertise || '';
        if (oldText.trim() && newText.trim() && oldText.trim() !== newText.trim()) {
          replacements[expertiseStartIdx + 1] = trackedParagraph(oldText, newText);
        }
      }
    }

    // Replace experience bullets — find bullet paragraphs after PROFESSIONAL EXPERIENCE
    if (expertiseEndIdx > -1) {
      const expParas = paras.slice(expertiseEndIdx + 1);

      // Match experience bullets from cvData to paragraphs
      // Group cvData experience bullets
      const allNewBullets = [];
      (cvData.experience || []).forEach(exp => {
        (exp.bullets || []).forEach(b => allNewBullets.push(b));
      });

      let bulletIdx = 0;
      expParas.forEach((p, i) => {
        const text = getParaText(p.xml).trim();
        // Detect bullet paragraphs — they have numPr (list formatting) or start with common patterns
        const hasBullet = p.xml.includes('<w:numPr>') || p.xml.includes('w:numId');
        if (hasBullet && text.length > 20 && bulletIdx < allNewBullets.length) {
          const newText = allNewBullets[bulletIdx];
          bulletIdx++;
          if (text !== newText) {
            const numPrMatch = p.xml.match(/<w:numPr>[\s\S]*?<\/w:numPr>/);
            const numPrXml = numPrMatch ? numPrMatch[0] : '';
            replacements[expertiseEndIdx + 1 + i] = trackedBullet(text, newText, numPrXml);
          }
        }
      });
    }
    ...
    if (Object.keys(replacements).length > 0) {
      // Rebuild XML with replacements
      let newXml = docXml;
      // Apply in reverse order to preserve positions
      const sortedIdxs = Object.keys(replacements).map(Number).sort((a, b) => b - a);
      sortedIdxs.forEach(idx => {
        const p = paras[idx];
        if (p) {
          newXml = newXml.substring(0, p.start) + replacements[idx] + newXml.substring(p.end);
        }
      });
      docXml = newXml;
    }
